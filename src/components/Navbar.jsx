...
import { LayoutDashboard, TrendingUp, ArrowDownCircle, ArrowUpCircle, History, LogOut, Shield, Settings, ChevronDown, User } from 'lucide-react'
  ...
  return (
    <>
      <style>{`
        ...
        .nv-profile{transition:background .2s}.nv-profile:hover{background:rgba(0,212,255,0.06)!important}
        .nv-dd{transition:background .2s,color .2s}.nv-dd:hover{background:#0f2040!important;color:#cbd5e1!important}
        .nv-out{transition:background .2s,color .2s}.nv-out:hover{background:rgba(239,68,68,0.1)!important;color:#ef4444!important}
        .nv-mob{transition:background .2s,color .2s}.nv-mob:hover{background:rgba(0,212,255,0.06)!important;color:#00d4ff!important}
        .nv-plink{transition:background .2s}.nv-plink:hover{background:#0f2040!important}
        ...
        .hb-bar{display:block;width:20px;height:2px;background:#94a3b8;border-radius:2px;transition:transform .3s cubic-bezier(.16,1,.3,1),opacity .2s,width .3s;transform-origin:center}
      ...
      <nav style={{ position:'fixed',top:0,left:0,right:0,zIndex:200,background:'rgba(2,8,23,.97)',backdropFilter:'blur(24px)',borderBottom:'1px solid #0f2040',fontFamily:"'Syne',sans-serif" }}>
        <div style={{ maxWidth:'1400px',margin:'0 auto',padding:'0 1.4rem',height:'62px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'1rem' }}>
          ...
          <div className="nv-desktop" style={{ display:'flex',alignItems:'center',gap:'.05rem',flex:1,justifyContent:'center' }}>
            {navItems.map(item => {
              ...
              return (
                <Link key={item.to} to={item.to} className="nv-link"
                  style={{ display:'flex',alignItems:'center',gap:'.38rem',padding:'0 .9rem',height:'62px',textDecoration:'none',fontSize:'.78rem',fontWeight:'600',letterSpacing:'.01em',whiteSpace:'nowrap',color:active?'#00d4ff':'#64748b',background:active?'rgba(0,212,255,0.07)':'transparent',borderBottom:`2px solid ${active?'#00d4ff':'transparent'}`,transition:'all .2s' }}>
                  <span style={{ color:active?'#00d4ff':isKyc?kycColor:'#64748b',display:'flex',transition:'color .2s' }}>{item.icon}</span>
                  {item.label}
            ...
            {profile?.is_admin && (
              <>
                <Link to="/admin" className="nv-link" style={{ display:'flex',alignItems:'center',gap:'.38rem',padding:'0 .9rem',height:'62px',textDecoration:'none',fontSize:'.78rem',fontWeight:'600',color:'#f59e0b',background:isActive('/admin')?'rgba(245,158,11,0.07)':'transparent',borderBottom:`2px solid ${isActive('/admin')?'#f59e0b':'transparent'}`,transition:'all .2s' }}>
                  <Settings size={15} /> Admin
                </Link>
                <Link to="/admin/kyc" className="nv-link" style={{ display:'flex',alignItems:'center',gap:'.38rem',padding:'0 .9rem',height:'62px',textDecoration:'none',fontSize:'.78rem',fontWeight:'600',color:'#f59e0b',background:isActive('/admin/kyc')?'rgba(245,158,11,0.07)':'transparent',borderBottom:`2px solid ${isActive('/admin/kyc')?'#f59e0b':'transparent'}`,transition:'all .2s' }}>
          ...
          <div style={{ display:'flex',alignItems:'center',gap:'.7rem',flexShrink:0 }}>
            <div className="nv-balance" style={{ background:'#0f2040',border:'1px solid #1e3a5f',borderRadius:'8px',padding:'.36rem .85rem',display:'flex',flexDirection:'column',alignItems:'flex-end' }}>
              ...
              <span style={{ color:'#00ff88',fontWeight:'700',fontSize:'.85rem',fontFamily:'DM Sans' }}>${profile?.balance?.toFixed(2)??'0.00'}</span>
            </div>
            ...
            <div className="nv-profbtn" ref={profileRef} style={{ position:'relative' }}>
              ...
              {profileOpen && (
                <div className="nv-dropdown" style={{ position:'absolute',top:'calc(100% + 8px)',right:0,background:'#030e1f',border:'1px solid #0f2a4a',borderRadius:'12px',padding:'.45rem',minWidth:'225px',boxShadow:'0 24px 48px rgba(0,0,0,.7)',zIndex:300 }}>
                  ...
                  <Link to="/profile" className="nv-plink" onClick={() => setProfileOpen(false)}
                    ...
                    <div style={{ display:'flex',alignItems:'center',gap:'.6rem' }}>
                      ...
                      <div style={{ flex:1,minWidth:0 }}>
                        ...
                        <p style={{ color:'#334155',fontFamily:'DM Sans',fontSize:'.74rem',margin:'.1rem 0 0',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{profile?.email}</p>
                      </div>
                    </div>
                    <div style={{ display:'flex',alignItems:'center',gap:'.3rem',marginTop:'.5rem',color:'#00d4ff',fontFamily:'DM Sans',fontSize:'.72rem' }}>
        ...
        <div className={`nv-mobile-wrap${mobileOpen?' nv-mobile-open':' nv-mobile-close'}`}
          ...
          <Link to="/profile" onClick={() => setMobileOpen(false)} className="nv-plink"
            style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'.9rem 1.4rem',borderBottom:'1px solid #0f2040',textDecoration:'none' }}>
            <div style={{ display:'flex',alignItems:'center',gap:'.6rem' }}>
              <div style={{ width:'36px',height:'36px',borderRadius:'50%',background:'linear-gradient(135deg,#00d4ff22,#00ff8822)',border:'1px solid #00d4ff44',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'1.2rem',flexShrink:0 }}>
                {avatarEmoji}
              </div>
              <div>
                ...
                <p style={{ color:'#00d4ff',fontFamily:'DM Sans',fontSize:'.7rem',margin:0 }}>View Profile →</p>
              </div>
            </div>
            <div style={{ background:'#0f2040',borderRadius:'8px',padding:'.35rem .8rem',textAlign:'right' }}>
              ...
              <p style={{ color:'#00ff88',fontFamily:'DM Sans',fontWeight:'700',fontSize:'.88rem',margin:0 }}>${profile?.balance?.toFixed(2)??'0.00'}</p>
            </div>
          </Link>
            ...
            {navItems.map((item, idx) => {
              ...
              return (
                <Link key={item.to} to={item.to} className="nv-mob" onClick={() => setMobileOpen(false)}
                  style={{ display:'flex',alignItems:'center',gap:'.7rem',padding:'.72rem 1rem',borderRadius:'8px',textDecoration:'none',fontFamily:'DM Sans',fontSize:'.88rem',color:active?'#00d4ff':'#64748b',background:active?'rgba(0,212,255,0.07)':'transparent',marginBottom:'.2rem',animation:`fadeIn .25s ${idx*0.04}s both` }}>
